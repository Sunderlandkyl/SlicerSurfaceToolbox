...
class MeshParseRule(ScriptedLoadableModule):
...
class MeshParseRuleWidget(ScriptedLoadableModuleWidget, VTKObservationMixin):
  ...
  def setup(self):
    ...
    if ruleTextNode is None:
      ...
      storageNode.SetFileName("E:\d\s\S4W\SurfaceToolbox\MeshParseRule\parcellation.qry.txt") #TODO
      storageNode.ReadData(ruleTextNode)
      slicer.mrmlScene.RemoveNode(storageNode)

    # Load widget from .ui file (created by Qt Designer)
    uiWidget = slicer.util.loadUI(self.resourcePath('UI/MeshParseRule.ui'))
  ...
  def onLoadQuery(self):
    ...
    self.logic.parseMeshRuleString(self._parameterNode)


class MeshParseRuleLogic(ScriptedLoadableModuleLogic):
  """Perform filtering
  """
  ...
  def parseMeshRuleString(self, parameterNode):
...
class MeshParseRuleVisitor(ast.NodeVisitor):
  """TODO
  """
...
class MeshParseRuleTest(ScriptedLoadableModuleTest):
  """
  ...
  def meshParseRule1(self):
    ...
    logic.parseMeshRuleString(parameterNode)
